...
export function Navbar() {
  ...
  const navItems = [
    { label: t("nav.calculator"), href: localizePath("/calculator") },
    { label: t("nav.projects"), href: localizePath("/projects") },
    { label: t("nav.products"), href: localizePath("/products") },
    ...
    <header className="sticky top-0 z-50 px-4 pt-4 sm:px-6">
      <nav className="relative mx-auto max-w-7xl rounded-2xl border border-white/[0.09] bg-[#090612]/75 px-4 shadow-[0_18px_70px_rgba(0,0,0,.38)] backdrop-blur-2xl sm:px-5">
        <div className="flex h-[74px] items-center">
          <a href={localizePath("/")} className="group mr-8 flex shrink-0 items-center gap-3 text-xl font-black tracking-[0.14em] text-white">
